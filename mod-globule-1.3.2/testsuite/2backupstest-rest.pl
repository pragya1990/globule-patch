...
# Cleaning up
#`rm -rf server?`; Already done by 2backuptest-master.pl
...
#init_origin_docs($origin0); Already done by 2backuptest-master.pl   

# Start servers
#start_server($server0);  # started by other
start_server($server1);
#start_server($server2);  # started by other
...
# Stop the origin
#stop_server($server0);
print "Ready to stop the origin and other backup...\n";
my_sleep(20);
