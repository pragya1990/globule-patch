...
use TestFunc qw($WIN32 $CWD $APACHE $SERVER $ORIGIN $REPLICA $REDIR $BACKUP
                ...
                $CONTAINS $LOCALHOST @my_hostnames init_servers 
                init_origin_docs start_server stop_server get_doc 
                get_doc_full update_doc flood_test my_exit my_sleep);
...
# server0 has 2 virtual host sections
my $vhost0 = {$ROLE     => $VHOST,
              $HOSTNAME => $hostname };
my $vhost1 = {$ROLE     => $VHOST,
              $HOSTNAME => "globule.revolutionware.net" };

# Localhost is always there, let the script know that our real hostname is
# also this systems name.
push @my_hostnames, $hostname, "globule.revolutionware.net";
...
# server0 contains vhost0 and vhost1
$server0->{$CONTAINS} = [$vhost0, $vhost1];
$vhost0->{$SERVER} = $server0;
$vhost1->{$SERVER} = $server0;

# vhost0 has origin0
$vhost0->{$CONTAINS} = [$origin0];
$origin0->{$SERVER} = $vhost0;

# vhost1 has replica0
$vhost1->{$CONTAINS} = [$replica0];
$replica0->{$SERVER} = $vhost1;
...
print "Getting replica1 and 2 1.html...\n";
...
# Some manual tests
get_doc_full($origin0, "/index.html");
get_doc_full($origin1, "/index.html");
get_doc($origin0, "1.html");
get_doc($replica0,  "1.html");
get_doc($replica2,  "1.html", 200);
get_doc_full($replica3,  "/index.html");
get_doc($origin0, "1.html");
