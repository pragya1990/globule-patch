...
use TestFunc qw($WIN32 $CWD $APACHE $SERVER $ORIGIN $REPLICA $REDIR $BACKUP
                ...
                stop_server get_doc get_doc_full update_doc flood_test my_exit
                my_sleep @my_hostnames);
...
#If you use other hostnames for your server, which are on the same machine
#add them to the my_hostnames list.
#push (@my_hostnames, "wilfred.cs.vu.nl");
